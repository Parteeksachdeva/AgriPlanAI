...
import { TrendingUp, TrendingDown, Minus, AlertCircle, Calendar, DollarSign, Activity, Sparkles, Warehouse, MapPin, Store } from 'lucide-react';
...
export function PricePrediction({ commodity, state }: PricePredictionProps) {
  ...
  const [seasonalTrend, setSeasonalTrend] = useState<SeasonalTrend | null>(null);
  const [mandiPrices, setMandiPrices] = useState<MandiPrice[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [daysAhead, setDaysAhead] = useState(7);
  const [activeTab, setActiveTab] = useState<'forecast' | 'seasonal' | 'mandis'>('forecast');
  ...
  const fetchSeasonalTrend = async () => {
    try {
      const response = await fetch(`http://localhost:8000/api/seasonal-trends/${encodeURIComponent(mandiCommodity)}/${encodeURIComponent(state)}`);
      if (response.ok) {
        const data = await response.json();
  ...
  const fetchMandiPrices = async () => {
    try {
      const response = await fetch(`http://localhost:8000/api/nearby-mandi-prices/${encodeURIComponent(mandiCommodity)}/${encodeURIComponent(state)}`);
      if (response.ok) {
        const data = await response.json();
  ...
  const getRecommendationStyle = (recommendation: string) => {
    switch (recommendation) {
      ...
      case 'STORE_AND_SELL_LATER':
        return { bg: 'bg-violet-500', light: 'bg-violet-50', text: 'text-violet-700', border: 'border-violet-200', icon: Warehouse };
      case 'WAIT':
      case 'HOLD':
  ...
  const getRecommendationLabel = (recommendation: string) => {
    switch (recommendation) {
      case 'SELL_NOW':
      case 'SELL_IMMEDIATELY': return 'Sell Immediately';
      case 'SELL_SOON': return 'Sell Soon';
      case 'STORE_AND_SELL_LATER': return 'Store & Sell Later';
      case 'HOLD': return 'Hold & Monitor';
      case 'HOLD_IF_STORAGE_AVAILABLE': return 'Hold if Storage Available';
          ...
          {storage && (
            <div className="bg-gradient-to-br from-violet-50 to-purple-50 rounded-2xl p-5 border border-violet-200">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h4 className="font-semibold text-violet-900">Storage Analysis</h4>
              </div>
              
              <div className="grid grid-cols-3 gap-4 mb-4">
                <div className="bg-white rounded-xl p-3 text-center">
                  <p className="text-xs text-slate-500 mb-1">Storage Cost</p>
                  <p className="font-bold text-slate-700">₹{storage.storage_cost}/q</p>
                  <p className="text-[10px] text-slate-400">for {daysAhead} days</p>
                </div>
                <div className="bg-white rounded-xl p-3 text-center">
                  <p className="text-xs text-slate-500 mb-1">Potential Gain</p>
                  <p className={cn("font-bold", storage.potential_gain > 0 ? 'text-emerald-600' : 'text-rose-600')}>
                    ₹{storage.potential_gain}/q
                  </p>
                </div>
                <div className="bg-white rounded-xl p-3 text-center">
                  <p className="text-xs text-slate-500 mb-1">Net Gain</p>
                  <p className={cn("font-bold", storage.net_gain > 0 ? 'text-emerald-600' : 'text-rose-600')}>
                    ₹{storage.net_gain}/q
                  </p>
                </div>
              </div>
              ...
              {storage.break_even_days > 0 && storage.break_even_days < 999 && (
                <div className="bg-violet-100 rounded-lg p-3">
                  <p className="text-sm text-violet-800">
                    <span className="font-semibold">Break-even:</span> Store for {storage.break_even_days} days to cover storage costs
                  </p>
                </div>
              )}
            </div>
        ...
        <div className="space-y-6">
          <div className="bg-gradient-to-br from-amber-50 to-orange-50 rounded-2xl p-5 border border-amber-200">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h4 className="font-semibold text-amber-900">Best Time to Sell</h4>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4 mb-4">
              <div className="bg-white rounded-xl p-4 text-center border-2 border-emerald-200">
                ...
                <p className="text-2xl font-bold text-emerald-600">{seasonalTrend.best_month.month}</p>
                <p className="text-sm text-slate-600">₹{formatIndianNumber(seasonalTrend.best_month.avg_price)}/q</p>
              </div>
              <div className="bg-white rounded-xl p-4 text-center">
                <p className="text-xs text-slate-500 mb-1">Worst Month</p>
                <p className="text-xl font-bold text-rose-600">{seasonalTrend.worst_month.month}</p>
                <p className="text-sm text-slate-600">₹{formatIndianNumber(seasonalTrend.worst_month.avg_price)}/q</p>
              </div>
            </div>

            <div className="bg-amber-100 rounded-lg p-3">
              <p className="text-sm text-amber-800">
                <span className="font-semibold">Price Difference:</span> {seasonalTrend.price_difference_pct}% higher in {seasonalTrend.best_month.month} vs {seasonalTrend.worst_month.month}
              </p>
            </div>
          </div>

          <div className="bg-white rounded-2xl border p-5">
            <h4 className="font-semibold text-foreground mb-4">Seasonal Strategy</h4>
            <div className="space-y-3 text-sm text-slate-600">
              <p>• Store your harvest if you can sell in {seasonalTrend.best_month.month}</p>
              <p>• Avoid selling in {seasonalTrend.worst_month.month} when prices are lowest</p>
            ...
            <>
              <div className="bg-gradient-to-br from-emerald-50 to-teal-50 rounded-2xl p-5 border border-emerald-200">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <h4 className="font-semibold text-emerald-900">Best Mandi to Sell</h4>
                </div>
                
                <div className="bg-white rounded-xl p-4">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-lg font-bold text-emerald-700">{mandiPrices[0].market}</p>
                      <p className="text-sm text-slate-500">Updated: {new Date(mandiPrices[0].date).toLocaleDateString()}</p>
                    </div>
                    <div className="text-right">
                      <p className="text-2xl font-bold text-emerald-600">₹{formatIndianNumber(mandiPrices[0].latest_price)}</p>
                ...
                {mandiPrices.length > 1 && (
                  <div className="mt-4 bg-emerald-100 rounded-lg p-3">
                    <p className="text-sm text-emerald-800">
                      <span className="font-semibold">Tip:</span> You can get ₹{formatIndianNumber(mandiPrices[0].latest_price - mandiPrices[mandiPrices.length-1].latest_price)}/q more by selling at {mandiPrices[0].market} instead of {mandiPrices[mandiPrices.length-1].market}
                    </p>
                  </div>
                )}
              </div>
              ...
              <div className="bg-white rounded-2xl border p-5">
                <h4 className="font-semibold text-foreground mb-4">All Mandi Prices</h4>
                <div className="space-y-2">
                  {mandiPrices.map((mandi, idx) => (
                    <div key={idx} className="flex items-center justify-between py-3 border-b last:border-0">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="font-medium text-slate-700">{mandi.market}</span>
                      </div>
                      <div className="text-right">
                        <p className="font-bold text-slate-900">₹{formatIndianNumber(mandi.latest_price)}</p>
                        <p className="text-xs text-slate-400">7-day avg: ₹{formatIndianNumber(mandi.avg_price_7d)}</p>
                      </div>
                    </div>
                  ))}
                </div>
